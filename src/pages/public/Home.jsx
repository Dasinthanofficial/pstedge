...
const BlueUnderline = () => (
  <svg className="absolute -bottom-1 md:-bottom-4 left-0 w-full h-3 md:h-5 text-blue-500 overflow-visible" viewBox="0 0 400 20" preserveAspectRatio="none">
    <motion.path 
...
const FAQItem = ({ question, answer, isOpen, onClick, index }) => {
  return (
    ...
    >
      ...
      >
        <span className={`font-semibold text-base sm:text-lg md:text-xl transition-colors duration-300 pr-4 ${isOpen ? 'text-gray-900' : 'text-gray-800 group-hover:text-blue-600'}`}>
          {question}
        </span>
        <motion.div 
          animate={{ rotate: isOpen ? 180 : 0 }} 
          transition={{ type: "spring", stiffness: 200, damping: 20 }}
          className={`flex-shrink-0 transition-colors duration-300 ${isOpen ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-500'}`}
        >
...
const ContactFormModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 sm:p-6">
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 md:top-6 md:right-6 text-gray-400 hover:text-black transition-colors bg-gray-100 rounded-full p-2 hover:bg-gray-200">
          <X size={20} />
        </button>
        <h2 className="text-2xl md:text-3xl font-bold mb-2 pr-8">Start a Project</h2>
        <p className="text-gray-500 mb-6 text-sm md:text-base">Fill out the form below and we'll get back to you within 24 hours.</p>
        ...
        {status.success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <input required placeholder="Business or Your Name" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm md:text-base focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" onChange={e => setFormData({...formData, businessName: e.target.value})} />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input required type="email" placeholder="Email Address" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm md:text-base focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" onChange={e => setFormData({...formData, email: e.target.value})} />
              <input required placeholder="Phone Number" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm md:text-base focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" onChange={e => setFormData({...formData, phone: e.target.value})} />
            </div>
            <select required className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm md:text-base focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-gray-700" onChange={e => setFormData({...formData, service: e.target.value})}>
            ...
            <textarea required rows="4" placeholder="Tell us about your project or goals..." className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm md:text-base resize-none focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" onChange={e => setFormData({...formData, projectDescription: e.target.value})}></textarea>
    ...
    <div className="w-full bg-white selection:bg-blue-100 selection:text-blue-900 relative overflow-hidden">
      ...
      <section className="relative pt-32 pb-12 md:pt-40 md:pb-16 px-4 sm:px-6 flex flex-col items-center text-center overflow-hidden min-h-[90vh] md:min-h-fit justify-center">
        ...
        >
          ...
          <motion.div 
            variants={itemVariants} 
            className="mt-16 md:mt-20 pt-8 md:pt-10 relative w-full overflow-hidden"
          >
      ...
      <section id="projects" className="py-16 md:py-24 px-4 sm:px-6 max-w-7xl mx-auto">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end gap-2 md:gap-4">
            ...
            <p className="text-gray-500 text-base md:text-lg md:ml-4 pb-1">Selected work from local businesses to international brands.</p>
          </div>
          {totalProjects > 4 && (
              ...
              >
                <Link to={`/projects/${item.slug}`} className="group block cursor-pointer">
                  ...
                  <div className="flex justify-between items-start px-1 md:px-0">
                    <div>
                      <h3 className="text-xl md:text-2xl font-bold text-gray-900 mb-1 md:mb-2 group-hover:text-blue-600 transition-colors">{item.title}</h3>
                      <p className="text-gray-500 text-sm md:text-base leading-relaxed pr-4 md:pr-8 line-clamp-2">{item.description}</p>
                    </div>
          ...
          >
            ...
            <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 bg-black/40 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-white/10 text-white">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] md:text-xs uppercase tracking-widest font-semibold text-gray-300">Founder & Developer</span>
              </div>
      ...
      <section className="pt-8 pb-4 md:py-12 px-4 sm:px-6 max-w-7xl mx-auto overflow-hidden">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-400 text-sm md:text-lg mb-6 md:mb-8 max-w-xl mx-auto">Let's create something extraordinary together. We look forward to hearing from you.</p>
        ...
        <div className="bg-[#050505] rounded-t-[2rem] rounded-b-3xl md:rounded-[2.5rem] p-6 sm:p-8 md:p-16 lg:p-20 text-white relative overflow-hidden flex flex-col justify-between md:min-h-[550px] border border-gray-900 shadow-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16 relative z-10 items-center">
            ...
            <div className="lg:col-span-5 flex justify-center lg:justify-end w-full mt-4 md:mt-0">
              ...
              >
                ...
                <div className="relative z-10">
                  <h3 className="text-2xl md:text-3xl font-black tracking-tight mb-1 text-white">PST EDGE<span className="text-blue-500">.</span></h3>
                  <p className="text-gray-400 text-xs md:text-sm font-medium mb-6 md:mb-8">Premium Web & Software Agency</p>
                  ...
                  <div className="space-y-4 md:space-y-6 mb-6 md:mb-8">
                    <div className="flex items-center gap-4 text-gray-300 group-hover:text-white transition-colors">
                      ...
                      <div>
                        <p className="text-[10px] md:text-xs text-gray-500 font-bold uppercase tracking-wider">Based In</p>
                        <p className="font-medium text-xs md:text-sm">Srilanka</p>
                      </div>
                    </div>
                    
                    {/* FIXED: TRUNCATED EMAIL CARD */}
                    <div className="flex items-center gap-4 text-gray-300 group-hover:text-white transition-colors w-full">
                      <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/5 flex items-center justify-center border border-white/10 text-blue-400 shrink-0">
                        <Mail size={16} className="md:w-[18px] md:h-[18px]" />
                      </div>
                      <div className="min-w-0 flex-1 pr-2">
                        <p className="text-[10px] md:text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Direct Contact</p>
                        <a 
                          href="mailto:dasinthanpathmanathan984@gmail.com" 
          ...
          >
            ...
            <div className="flex items-center gap-6">
              <p className="text-gray-600 text-xs md:text-sm hidden md:block">© {new Date().getFullYear()} PST EDGE. All rights reserved.</p>
              <div className="flex items-center gap-4 text-gray-500">
                <a href="#" className="hover:text-white transition-colors hover:-translate-y-1 transform duration-200"><Github size={18} className="md:w-[20px] md:h-[20px]"/></a>
                <a href="#" className="hover:text-white transition-colors hover:-translate-y-1 transform duration-200"><Linkedin size={18} className="md:w-[20px] md:h-[20px]"/></a>
              </div>
            </div>
          </motion.div>
